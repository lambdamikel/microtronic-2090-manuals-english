...
/* Annotated listing (6th "Explanation" column): use fixed layout + a colgroup
   so the explanation column keeps a fixed share and wraps like the German
   original, instead of being squeezed by the rigid nowrap code columns. */
.ovr.body table.timer.ann { table-layout:fixed; word-wrap:break-word; }
/* Label (1st) and Jump-to (5th) columns hold quoted identifiers like „SPGEW"
   that are as wide as the narrow column — shrink their font + tighten side
   padding so they don't run into the cell borders. */
.ovr.body table.timer.ann td:nth-child(1),
.ovr.body table.timer.ann td:nth-child(5) {
  font-size:85%; padding-left:.5mm; padding-right:.5mm; }
...
# The sheet is no longer A4: this book's pages are squarish, and the two loose
# game-board inserts are taller again. @page carries the size, with a named page
# for the odd ones out, so the PDF has no white band under every sheet.
SHEET_CSS = """
@page {{ size:210mm {h:.2f}mm; margin:0; }}
@page tall {{ size:210mm {ht:.2f}mm; margin:0; }}
.page.tall {{ page:tall; }}
"""
...
def _mask_div(bbox, ppm, pad_mm, color=None):
    ...
    return (
        f'  <div class="mask" '
        f'style="{bg}left:{x:.2f}mm;top:{y:.2f}mm;'
        f'width:{w:.2f}mm;height:{h:.2f}mm"></div>'
    )
...
def render_page(page_idx, layout, en_map, bg_rel_path):
    ...
    parts = []
    # The sheet is 210 mm wide; its height follows the scan, so a square cover
    # is not stretched into an A4 frame. Inline, so it beats the print CSS too.
    pw, ph = layout.get("page_size_px", [2480, 3507])
    h_mm = 210.0 * ph / pw if pw else 297.0
    style = f' style="height:{h_mm:.2f}mm"' if pw else ""
    # a sheet noticeably taller than the book's own gets its own @page size
    tall = " tall" if h_mm > 240.0 else ""
    parts.append(f'<section class="page{tall}" data-page="{page_idx}"{style}>')
    # bg_crop_px trims the scan's edges (the covers' white platen sliver): blow
    # the image up and pull it out past the sheet, which .page's overflow:hidden
    # then clips. Overlay coordinates were already shifted to match.
    crop = layout.get("bg_crop_px")
    if crop:
        cl, ct, cr, cb = crop
        bs = (f'style="width:{100.0 * (pw + cl + cr) / pw:.4f}%;'
              f'height:{100.0 * (ph + ct + cb) / ph:.4f}%;'
              f'left:{-100.0 * cl / pw:.4f}%;top:{-100.0 * ct / ph:.4f}%;'
              f'right:auto;bottom:auto" ')
    else:
        bs = ""
    parts.append(f'  <img class="bg" {bs}src="{bg_rel_path}" alt="">')
    ...
    for i, b in enumerate(blocks):
        ...
        mc = b.get("mask_color")
        for f in units:
            parts.append(_mask_div(f, ppm, pad, mc))
        # tight_masks: exact rectangles with (almost) no pad, for masking text
        # that sits in a narrow gap between preserved figures.
        for f in b.get("tight_masks", []):
            parts.append(_mask_div(f, ppm, 0.0, mc))
...
def main():
    ...
    ap.add_argument("--title", help="document <title> (also the PDF's title)")
    ap.add_argument(
        "--sheet-crop", metavar="PAGES:L,T,R,B",
        help='Trim every listed page to one sheet size, e.g. "4-64:0,0,9,1062".'
             " The body pages are A4 scans of a squarish booklet - the bottom"
             " third is blank - so trimming them makes every sheet match the"
             " covers. Pages that set bg_crop_px themselves (the covers) are"
             " left alone.")
    ap.add_argument(
        "--make-templates",
        action="store_true",
    ...
    sheet_heights = []
    sheet_pages, sheet_crop = set(), None
    if args.sheet_crop:
        spec, nums = args.sheet_crop.split(":")[::-1][0], args.sheet_crop.split(":")[0]
        sheet_crop = [int(v) for v in spec.split(",")]
        for part in nums.split(","):
            if "-" in part:
                a, b = (int(v) for v in part.split("-"))
                sheet_pages.update(range(a, b + 1))
            else:
                sheet_pages.add(int(part))
    if "-" in args.pages:
    ...
    for n in page_nums:
        ...
        if n in sheet_pages and "bg_crop_px" not in layout:
            cl, ct, cr, cb = sheet_crop
            w, h = layout.get("page_size_px", [2480, 3507])
            layout["page_size_px"] = [w - cl - cr, h - ct - cb]
            layout["px_per_mm"] = (w - cl - cr) / 210.0
            layout["bg_crop_px"] = list(sheet_crop)
        img_abs = root / "pages" / f"page-{n:02d}.png"
        out_dir = Path(args.out).resolve().parent
        bg_rel = os.path.relpath(img_abs, out_dir)
        pw_, ph_ = layout.get("page_size_px", [2480, 3507])
        sheet_heights.append(210.0 * ph_ / pw_ if pw_ else 297.0)
        sections.append(render_page(n, layout, en_map, bg_rel))

    from collections import Counter
    ordinary = [h for h in sheet_heights if h <= 240.0] or sheet_heights
    sheet_h = Counter(round(h, 2) for h in ordinary).most_common(1)[0][0]
    tall = [h for h in sheet_heights if h > 240.0]
    sheet_tall = Counter(round(h, 2) for h in tall).most_common(1)[0][0] if tall else 297.0

    out_path = Path(args.out)
    out_path.write_text(
        "<!doctype html>\n<html><head><meta charset='utf-8'>"
        f"<title>{args.title or f'Microtronic 2090 — pages {args.pages}'}</title>"
        + "<style>" + SHEET_CSS.format(h=sheet_h, ht=sheet_tall) + "</style>"
