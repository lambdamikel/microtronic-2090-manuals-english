...
"""Turn the raw build_page_html.py output (part2.html) into a GitHub-Pages-ready site:
  ...
  * prepend a header with the title, the copyright / permission notice, and a clickable
    table of contents (chapter -> page anchor) read from anl2094/contents.json.

Usage:  python3 anl2094/tools/make_site.py computer-games.html
        (edits the file in place)

The chapter list uses the manual's PRINTED page numbers; for book 2094 the offset is +3,
i.e. printed page P is the (P+3)-th physical sheet -> anchor #page-{P+3}. Category
...
DOWNLOADS = """
<div class="dl">
  <a href="Busch_Microtronic_2094_Computer_Games_EN.pdf">&#128214; Computer Games &mdash; PDF (66&nbsp;pp)</a>
  <a href="index.html">&#8599; 2090 Part&nbsp;1 (online)</a>
  <a href="part2.html">&#8599; 2090 Part&nbsp;2 (online)</a>
...
def build_header(contents):
    ...
    for title, page in contents:
        if page is None:                     # category header spanning both columns
            ...
            continue
        anchor = f"page-{page + 3}"          # printed page P -> physical sheet P+3
    ...
    return (HEADER_CSS
            ...
            + '<h1>Busch Microtronic 2094 &mdash; Computer Spiele / Computer Games (English)</h1>'
            + '<p class="lead">25 games and experiments to program yourself '
              'for Microtronic 2090.</p>'
            + DOWNLOADS + NOTICE + toc + '</div>\n')

def main():
    path = pathlib.Path(sys.argv[1] if len(sys.argv) > 1 else "computer-games.html")
    contents = json.load(open(path.parent / "anl2094" / "contents.json", encoding="utf-8"))
    doc = path.read_text(encoding="utf-8")

    doc = doc.replace('<img class="bg"', '<img loading="lazy" class="bg"')
    # anchor each page section
    for line in [l for l in doc.splitlines() if '<section class="page' in l
                 and 'data-page=' in l]:
        n = line.split('data-page="')[1].split('"')[0]
        doc = doc.replace(line, line.replace('data-page=',
                                             f'id="page-{n}" data-page=', 1), 1)
